        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if(cookie.equals("")) {
                    ...
                    if(remote_products_version == -1){ // cookie on server side is no longer alive
                        String mail = pref.getString(SHARED_PREFERENCES_EMAIL, null);
                        String password = pref.getString(SHARED_PREFERENCES_PASSWORD, null);
                        if(mail == null || password == null){
                            Intent intent = new Intent(SpinnerActivity.this, Log_in_Activity.class);
                            startActivity(intent);
                            return;
                        }
                        ServerHelper.ServerResponse response = helper.login(mail, password);
                    ...
                    remote_products_version = helper.getProductsVersion();
